module.exports = () => {
  // PROJECT PRIORITY START
  /**
   * @swagger
   * components:
   *  schema:
   *    project-priority-list:
   *      type: object
   *      properties:
   *        status:
   *          type: string
   *          description: The status of the request.
   *        statusCode:
   *           type: integer
   *           description: The status code.
   *        data:
   *          type: object
   *          properties:
   *        message:
   *          type: string
   *          description: A feedback message of the request.
   *      example:
   *        status: SUCCESS
   *        statusCode: 200
   *        message: Priority list found.
   *        data:
   *          PriorityID: 1
   *          PriorityName: "Low"
   *          CreateDate: 14/05/2024
   * */
  /**
   * @swagger
   * /api/project-priority:
   *    get:
   *      summary: List priorities item .
   *      tags: [Project Priority]
   *      security:
   *        - bearerAuth: []
   *      responses:
   *        200:
   *          description: The priority list.
   *          content:
   *            application/json:
   *              schema:
   *                $ref: "#/components/schema/project-priority-list"
   *        422:
   *          description: Unprocessable Entity.
   *          content:
   *            application/json:
   *              schema:
   *                example:
   *                  status: false
   *                  statusCode: 422
   *                  data: {}
   *                  message: Error! priority not found
   *        401:
   *          description: Access Denied.
   *          content:
   *            application/json:
   *              schema:
   *                example:
   *                  status: false
   *                  statusCode: 401
   *                  data: {}
   *                  message: Invalid Token
   */
  /**
   * @swagger
   * /api/project-priority:
   *    post:
   *      summary: Add Project Priority .
   *      tags: [Project Priority]
   *      security:
   *        - bearerAuth: []
   *      requestBody:
   *        requires: true
   *        content:
   *         application/json:
   *            schema:
   *              type: object
   *              properties:
   *                PriorityName:
   *                  type: string
   *                  description: name of priority-label.
   *                Colorcode:
   *                  type: string
   *                  description: hexcode.
   *                TaskgroupID:
   *                  type: int
   *                  description: taskgroup id.
   *              example:
   *                  PriorityName: Not Urgent
   *                  Colorcode: #ffffff
   *                  TaskgroupID: 22
   *      responses:
   *        202:
   *          description: project added.
   *          content:
   *            application/json:
   *              schema:
   *                example:
   *                  status: true
   *                  statusCode: 202
   *                  data: {}
   *                  message: Priority Added successfully
   *        401:
   *          description: Access Denied.
   *          content:
   *            application/json:
   *              schema:
   *                example:
   *                  status: false
   *                  statusCode: 401
   *                  data: {}
   *                  message: Invalid Token
   */
  /**
   * @swagger
   * /api/project-priority/{id}:
   *    delete:
   *      summary: Delete Project priority .
   *      tags: [Project Priority]
   *      security:
   *        - bearerAuth: []
   *      parameters:
   *        - in: path
   *          name: id
   *          schema:
   *            type: string
   *            required: true
   *            description: The project priority id.
   *      responses:
   *        200:
   *          description: priority deleted.
   *          content:
   *            application/json:
   *              schema:
   *                example:
   *                  status: true
   *                  statusCode: 200
   *                  data: {}
   *                  message: priority deleted successfully
   *        422:
   *          description: Unprocessable Entity.
   *          content:
   *            application/json:
   *              schema:
   *                example:
   *                  status: false
   *                  statusCode: 422
   *                  data: {}
   *                  message: Error! priority not deleted
   *        401:
   *          description: Access Denied.
   *          content:
   *            application/json:
   *              schema:
   *                example:
   *                  status: false
   *                  statusCode: 401
   *                  data: {}
   *                  message: Invalid Token
   */
  // PROJECT PRIORITY END
}
